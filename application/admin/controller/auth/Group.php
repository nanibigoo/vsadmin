...
class Group extends Backend
{
    ...
    public function add()
    {
        if ($this->request->isPost())
        {
            ...
            $is_hospital = $params['is_hospital'];
            unset($params['is_hospital']);
            ...
            if ($params)
            {
                $group = $this->model->create($params);
                if($is_hospital){
                    $this->saveHospitalInfo($group);
                }
                $this->success();
            }
            $this->error();
    ...
    public function edit($ids = NULL)
    {
        ...
        $is_hospital = $this->isHospital($row['id']);

        if ($this->request->isPost())
        {
            $params = $this->request->post("row/a", [], 'strip_tags');
            $is_hospital2 = $params['is_hospital'];
            unset($params['is_hospital']);
            ...
            {
                $row->save($params);
                if($is_hospital2 != $is_hospital){
                    $data['name']=$params['name'];
                    $data['id']=$row->id;
                    $this->saveHospitalInfo($row);
                }
                $this->success();
            }
            $this->error();
        ...
        $this->view->assign("is_hospital", $is_hospital);
     ...
     */
    private function saveHospitalInfo($info)
    {
        $hospital = model('hospital')->where('group_id',$info['id'])->find();
        if( !$hospital ){
            $data = [
                'group_id'=>$info['id'],
                'title'=>$info['name']
            ];
            Db::name('hospital')->insert($data);
        }else{
            $data = [
                'status'=> $hospital['status'] ? 0 : 1,
                'title'=>$info['name']
            ];
            Db::name('hospital')->where('group_id',$info['id'])->update($data);
        }
    }
    /**
     * 是否医院
     */
    private function isHospital($id)
    {
        $map=[
            'group_id'=>$id,
            'status'=>1
        ];
       return  model('hospital')->where($map)->find() ? 1 : 0;
    }
    /**
     * 删除角色组时同步删除医院(is_deleted改为1，status改为0)
     */
    private function deleteHospital($ids)
    {
        $data = [
            'is_deleted'=> 1,
            'status'=> 0
        ];
        Db::name('hospital')->where('group_id','in',$ids)->update($data);
    }
}
